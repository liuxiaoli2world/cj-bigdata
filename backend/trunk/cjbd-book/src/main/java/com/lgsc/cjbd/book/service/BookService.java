...
public class BookService {
	 ...
	 * @return
	 */
	public boolean checkIsbn(String realIsbn) {
		Book book = new Book();
		book.setRealIsbn(realIsbn);
		return Pattern.matches("^(\\d[- ]*){9}[\\dxX]$", realIsbn) && (bookMapper.select(book) != null);
	}
	...
	public int insertBook(Book book, Long channelId, Long menuId, String imageUrl) {
		...
		//旧版
		//book.setRealIsbn(book.getIsbn());
		
		int num = bookMapper.insertSelective(book);
		//更新isbn =  realIsbn+"_"+bookId
		book.setIsbn(book.getRealIsbn()+"_"+book.getBookId());
		num = bookMapper.updateByPrimaryKeySelective(book);
		if (num <= 0) {
			return 0;
		}
	 ...
	 * @return
	 */
	public boolean bookIsExist(String realIsbn,String bookName) {
		Book record = new Book();
		record.setRealIsbn(realIsbn);
