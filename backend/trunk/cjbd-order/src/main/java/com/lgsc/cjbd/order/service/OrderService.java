...
public class OrderService {
	 ...
	 */
	@Transactional
	public int deleteByOrderNo(String orderNo, String scene) {
		int result = 0;
		...
		if (order != null) {
			if ("portal".equals(scene)) {// 如果是门户删除
				Order param = new Order();
				param.setOrderId(order.getOrderId());
				param.setIsDisplay((short) 0);
				result = orderMapper.updateByPrimaryKeySelective(param);
			} else if ("bg".equals(scene)) {// 如果是后台删除
	...
	public PageInfo<Map<String, Object>> selectBy(Integer pageNum, Integer pageSize, String scene, Long userId, String orderType, String condition,
			Date orderDate) {
		// 将超时订单改为支付过时
		...
		if ("portal".equals(scene)) {// 如果是门户查询
			list = orderMapper.selectBy(userId, (short) 1, orderType, condition, dateString);// 只查询设置为显示的订单
		} else if ("bg".equals(scene)) {// 如果是后台查询
			list = orderMapper.selectBy(userId, null, orderType, condition, dateString);// 查询所有订单
		}
