...
public class UserService {
	...
	public Map<String, String> insertSelective(User user) {
		...
		if (record != null && record.getUsername().equals(user.getUsername())) {
			...
			String salt = UUID.randomUUID().toString().substring(0, 5).replace("-", "");
	...
	public int disableUser(long userId) {
		...
		try {
			new MailUtil(sendFrom, javaMailSender).SendMail("长江出版社", "您异常登录的ip过多账号被封了", record.getEmail());
		} catch (Exception e) {
			logger.error(e);
		}
		...
		if (user != null) {
			if (user.getEnable().equals(Short.parseShort("1"))) {
				...
				if (exceptionLoginService.IfSendEmail() > 0) {
					try {
						new MailUtil(sendFrom, javaMailSender).SendMail("长江出版社", "您异常登录的ip过多账号被封了", user.getEmail());
					} catch (Exception e) {
						return map;
					}
				}
